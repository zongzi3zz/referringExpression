...
class ReferDataset(data.Dataset):
    ...
    def __init__(self, data_root, split_root='data', dataset='referit', imsize=256,
        ...
        elif self.dataset == 'visual_genome':
            self.dataset_root = osp.join(self.data_root,'VG')
            self.im_dir = osp.join(self.data_root,'vg_images')
        else:   ## refcoco, etc.
...
class VgDataset(data.Dataset):
    ...
    def __init__(self, datacsv,data_root, split_root='data', dataset='referit', imsize=256,
                 transform=None, augment=False, return_idx=False, testmode=False,
                 split='train', max_query_len=128, lstm=False, bert_model='bert-base-uncased',
                 ):
        self.images = []
        ...
        self.augment=augment
        self.return_idx=return_idx
        self.dataset_csv =datacsv#"./data/vg_split/csv_dir/vgtrain.csv"
        if self.dataset == 'visual_gemone':
            #数据集路径
            self.dataset_root = '/home/B/ldy/zsg_hzz/zsgnet-pytorch/hdata/'
            #图片路径
            self.im_dir = "/home/B/ldy/zsg_hzz/zsgnet-pytorch/hdata"

        self.datacsv = pd.read_csv(self.dataset_csv)
    def exists_dataset(self):
        return osp.exists(osp.join(self.split_root, self.dataset))
    ...
    def pull_item(self, idx):
        ...
        vgdata['bbox'] = vgdata.bbox.apply(
            lambda x:ast.literal_eval(x) if not isinstance(x,list) else(x))

        sample = vgdata['phrase'].iloc[0]
        if sample[0] == '[':
            trn_data['phrase'] =vgdata['phrase'].apply(
                lambda x: ast.literal_eval(x))
        
        vgdata['x1'] = vgdata.bbox.apply(lambda x: x[0])
        vgdata['y1'] = vgdata.bbox.apply(lambda x: x[1])
        vgdata['x2'] = vgdata.bbox.apply(lambda x: x[2])+vgdata.bbox.apply(lambda x: x[0])
        vgdata['y2'] = vgdata.bbox.apply(lambda x: x[3])+vgdata.bbox.apply(lambda x: x[1])
        vgdf = vgdata[['image_fpath',
                               'x1', 'y1', 'x2', 'y2', 'phrase']]
        annotation_list=vgdf.iloc[idx]
